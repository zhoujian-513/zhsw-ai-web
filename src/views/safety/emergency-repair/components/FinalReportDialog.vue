        ...
        <ElRow :gutter="20">
          ...
          <ElCol :span="12">
            <ElFormItem label="终报人">
              <ElInput v-model="finalReporter" disabled />
            </ElFormItem>
          </ElCol>
        </ElRow>
...
import type { EmergencyRepairItem, RootCauseType, RootCauseOption } from '@/types/emergency-repair'
import { finalReport } from '@/api/emergency-repair'
...
const formRules = {
  ...
  rootCauseType: [{ required: true, message: '请选择根本原因分析', trigger: 'change' }],
...
watch(() => props.data, (val) => {
  if (val) {
    ...
    formData.rootCauseType = val.rootCauseType || ('' as RootCauseType)
    formData.rootCauseOption = val.rootCauseOption || ('' as RootCauseOption)
    formData.finalMeasures = val.finalMeasures || ''
    formData.attachments = [...val.finalAttachments]
    ...
    // 设置级联选择值
    if (val.rootCauseType && val.rootCauseOption) {
      rootCause.value = [val.rootCauseType, val.rootCauseOption]
    } else {
      rootCause.value = []
    }
  }
    ...
    if (valid) {
      ...
      try {
        const res = await finalReport({
          ...
          finalReporter: finalReporter.value
        })
        if (res.code === 200) {
