  ...
  >
    ...
    <div v-if="currentStep === 0" class="step-content">
      <div class="tip-text">
        ...
        <p>2. 模板中包含以下字段：所属厂站、设备名称、设备编码、设备类型、设备注册代码、使用证编号等</p>
    ...
    <div v-if="currentStep === 1" class="step-content">
      <div class="tip-text">
        <p>请按照以下要求填写数据：</p>
        <ul>
          <li>所属厂站：必须填写系统中已存在的厂站名称</li>
          <li>设备名称：必填，长度不超过50个字符</li>
          <li>设备编码：必填，不能重复</li>
          <li>设备类型：必填，如：锅炉、压力容器、电梯等</li>
          <li>设备注册代码：必填，格式为：TSxxxxx</li>
          <li>使用证编号：必填</li>
          <li>生产日期、校验日期：格式为 YYYY-MM-DD</li>
          <li>使用状态：填写"在用"或"停用"</li>
        </ul>
      </div>
...
import { downloadImportTemplate, importSpecialEquipment } from '@/api/special-equipment'
import type { ImportResult } from '@/types/special-equipment'
...
const handleUpload = async () => {
  ...
  try {
    const res = await importSpecialEquipment(selectedFile.value)
