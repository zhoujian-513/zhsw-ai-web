...
import type { GeoJsonData, GeoFeature } from '@/types/map'

export interface ProjectionFnParam {
...
// 生成地图3D模型
export function generateMapObject3D(mapdata: GeoJsonData, projectionFnParam: ProjectionFnParam) {
  ...
  basicFeatures.forEach((basicFeatureItem: GeoFeature) => {
    ...
    const featureCoords: any = basicFeatureItem.geometry.coordinates
    const featureCenterCoord: any =
      basicFeatureItem.properties.centroid && projectionFn(basicFeatureItem.properties.centroid)
    const featureName: string = basicFeatureItem.properties.name

    if (featureCenterCoord) {
      label2dData.push({ featureCenterCoord, featureName })
    }
...
export const draw2dLabel = (coord: [number, number], proviceName: string) => {
  if (coord && coord.length) {
    const innerHTML = `<div style="color: #fff; font-size: 12px; pointer-events: none;">${proviceName}</div>`
...
export const drawSpot = (coord: [number, number]) => {
  if (coord && coord.length) {
    ...
    const ring: any = new THREE.Mesh(ringGeometry, ringMaterial)
