...
<script setup lang="ts">
  ...
  const generateChartOptions = (isInitial = false): EChartsOption => {
    ...
    if (props.yAxis && props.yAxis.length > 0) {
      // 使用自定义多纵坐标
      yAxisConfig = props.yAxis.map((axis: any, index: number) => ({
        type: 'value',
        ...axis,
        axisLabel: getAxisLabelStyle(props.showAxisLabel),
        axisLine: getAxisLineStyle(props.showAxisLine),
        splitLine: index === 0 ? getSplitLineStyle(props.showSplitLine) : { show: false }
      }))
    } else {
      // 默认单纵坐标
      yAxisConfig = {
    ...
    const options: EChartsOption = {
      ...
      grid: getGridWithLegend(props.showLegend && isMultipleData.value, props.legendPosition, {
        top: 15,
        right: props.yAxis && props.yAxis.length > 1 ? 60 : 15,
        left: props.yAxis && props.yAxis.length > 1 ? 60 : 0
      }),
    ...
    if (isMultipleData.value) {
      ...
      options.series = multiData.map((item, index) => {
        const itemColor = item.color || getColor(props.colors[index], index)
        ...
        return createSeriesItem({
          ...
          areaStyle,
          yAxisIndex: (item as any).yAxisIndex
        })
      })
    } else {
      // 单数据情况
